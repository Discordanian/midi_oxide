...
struct MidiNote
{
    uint8_t nKey = 0; // 7 bit
    uint8_t nVelocity = 0; // 7 bit
    uint32_t nStartTime = 0; // 4 7bit -> 28bit
    uint32_t nDuration = 0; // 4 7bit -> 28bit
};
...
public:
    ...
    bool ParseFile(const std::string& sFileName)
    {
        ...
        uint32_t nFileID = Swap32(n32);  // MThd
        ifs.read((char*)&n32, sizeof(uint32_t));
        uint32_t nHeaderLength = Swap32(n32); // Length of Header
        ifs.read((char*)&n16, sizeof(uint16_t));
        uint16_t nFormat = Swap16(n16); // Format of the file
        ifs.read((char*)&n16, sizeof(uint16_t));
        uint16_t nTrackChunks = Swap16(n16); // number of Track Chunks
        ifs.read((char*)&n16, sizeof(uint16_t));
        uint16_t nDivision = Swap16(n16); // The tick division
        ...
        {            
            ...
            // Read Track Header
            ifs.read((char*)&n32, sizeof(uint32_t));
            uint32_t nTrackID = Swap32(n32); // MTrk
            ifs.read((char*)&n32, sizeof(uint32_t));
            uint32_t nTrackLength = Swap32(n32); // Track length in bytes.  

            /*
                Assuming track length is this  same variable lenght, it's going to be 28 bits long
                Which maxes out at 268435456 bytes.  268,435,456 so aprox 268Megs per track
            */
